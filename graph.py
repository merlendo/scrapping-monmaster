...
for lieu in lieux:
    ...
    taux_acces = f"{round(lieu["tauxAcces"], 2)*100}%" if lieu["tauxAcces"] else "[NA]"
    popup_html += f"<p>Taux d'accés : <b>{taux_acces}</b></p>"
    dernier_appele = lieu["rangDernierAppele"] if lieu["rangDernierAppele"] else "[NA]"
    nb_candidature = lieu["nbCandidateuresConfirmees"] if lieu["nbCandidateuresConfirmees"] else "[NA]"
    popup_html += f"<p>Rang dernier appelé : {dernier_appele} sur {nb_candidature} candidatures</p>"
    popup_html += f"<p>{lieu["commentaire"]}</p>" if lieu["commentaire"] else "<p>Pas de commentaire</p>"
